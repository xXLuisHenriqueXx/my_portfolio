...
const card = tv({
  slots: {
    container:
      "relative flex justify-center items-center w-full h-[80%] my-4 bg-backgroundIcon rounded-sm overflow-hidden",
    containerBox: "flex w-full h-full transition-transform duration-300",
    containerContent:
      "flex flex-col shrink-0 justify-center items-center w-full h-full",
    ...
    technologiesText:
      "text-xs 2xl:text-base font-semibold text-highlitedColor text-center py-1 px-2 bg-backgroundCard rounded-sm",
    descriptionText:
      "text-sm 2xl:text-base text-lightColor font-light text-wrap text-center mx-2",
    image: "w-52 md:w-64 2xl:w-80 h-auto object-cover my-4 cursor-pointer",
    button:
      "absolute top-1/2 transform -translate-y-1/2 bg-backgroundCard p-2 rounded-full",
  },
  variants: {
    button: {
      left: {
...
const projectsData = [
  {
    ...
    title: "PomodoroStudy",
    description:
      "Aplicativo Fullstack móvel que ajuda na organização estudantil.",
  ...
  },
  {
    ...
    title: "EcoMap",
    description:
      "Projeto de aplicativo, que consiste em uma ferramenta que busca locais próximos para descarte de lixo.",
          ...
          return (
            <div key={project.id} className={containerContent()}>
              ...
              <p className={createdText()}>
                Projeto criado em: <b>{project.date}</b>
              </p>
